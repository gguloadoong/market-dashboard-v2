...
**PR 필수** — 아래 조건 중 하나라도 해당:
- 새 기능 (`feat:`)
- 3개 이상 파일을 수정하는 버그 수정
- API / 데이터 로직 변경

**PR 불필요 — main 직접 커밋 허용**:
- `refactor:` / `docs:` / `chore:`
- 1~2개 파일 수정하는 단순 `fix:` (오타, 상수, 스타일)

**배치 원칙**: 같은 날 같은 기능 영역의 `fix:` 여러 개 → 하나의 PR로 묶는다.
PR을 자주 만들면 리뷰 봇 할당량이 소진된다. PR 하나당 의미 있는 단위여야 한다.

### PR 절차 (feat: / 대형 fix: 해당)
